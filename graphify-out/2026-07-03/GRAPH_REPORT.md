...
Cohesion: 0.11
Nodes (19): RiskTable(), SEVERITY_COLOR, CARDS, StatsGrid(), Dashboard(), healthScore(), LoadState, SECURITY_SEVERITY_COLOR (+11 more)
...
Cohesion: 0.29
Nodes (7): Path, main(), FakeGraph, _git(), test_build_hotspots_falls_back_to_complexity_only_without_git(), test_build_hotspots_ranks_churn_times_complexity(), test_build_hotspots_unavailable_when_graph_empty()
...
Cohesion: 0.09
Nodes (30): Proven-tool SAST integration: Bandit and Ruff security rules.  The builtin regex, All external-tool findings plus per-tool availability flags., Run a tool and parse its stdout as JSON; None on any failure.      Non-zero exit, Bandit findings for *repo_path*, or None if the tool is unusable., Ruff flake8-bandit (S) findings, or None if the tool is unusable., _rel(), run_bandit(), run_external_tools() (+22 more)
...
Cohesion: 0.25
Nodes (9): llm_provider(), The effective LLM provider (DB config override, else ``LLM_*`` env)., llm_config_models(), llm_config_pull(), llm_config_put(), LlmConfigUpdate, ModelPull, LLM configuration + local-model management (plan Phase C).  Free/local providers (+1 more)
